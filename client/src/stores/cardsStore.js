...
export const useCardsStore = defineStore('cards', () => {
  ...
  async function fetchCards(columnId) {
    const { data, error } = await supabase.from('cards').select('*').eq('column_id', columnId).order('position', { ascending: true })
    if (error) throw error
    cardsByColumn.value[columnId] = data || []
  }

  async function createCard(columnId, title, description) {
    const maxPos = cardsByColumn.value[columnId]?.reduce((max, c) => Math.max(max, c.position ?? 0), -1) ?? -1
    const { data, error } = await supabase.from('cards').insert([{ column_id: columnId, title, description, position: maxPos+1 }]).select()
    if (error) throw error
  ...
  return { cardsByColumn, fetchCards, createCard, updateCard, moveCard }
})
